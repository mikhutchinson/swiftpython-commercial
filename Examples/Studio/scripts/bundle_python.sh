...
# realpath resolves even broken/nonexistent targets, so escaping links are caught
# whether or not their destination exists.
FW="$FRAMEWORKS_DIR/Python.framework"
realpath_py() { /usr/bin/python3 -c 'import os,sys;print(os.path.realpath(sys.argv[1]))' "$1" 2>/dev/null; }
FW_REAL="$(realpath_py "$FW")"; [ -z "$FW_REAL" ] && FW_REAL="$FW"
find "$FW" -type l | while IFS= read -r link; do
    resolved="$(realpath_py "$link")"
    case "$resolved" in
        "$FW_REAL"/*|"$FW_REAL") : ;;   # resolves inside the framework — keep
        *) rm -f "$link"; echo "    removed escaping/broken symlink: ${link#$APP_DIR/}" ;;
    esac
done
